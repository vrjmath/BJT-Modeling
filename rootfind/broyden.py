...
def broyden(g, x0, threshold, eps, ls=False, name="unknown", device='cpu'):
    ...
    x_est = x0.clone().to(device)           # (bsz, 2d, L')
    gx = g(x_est)                           # (bsz, 2d, L')
    nstep = 0
    tnstep = 0

    # For fast calculation of inv_jacobian (approximately)
    Us = torch.zeros(bsz, total_hsize, seq_len, threshold).to(device)     # One can also use an L-BFGS scheme to further reduce memory
    VTs = torch.zeros(bsz, threshold, total_hsize, seq_len).to(device)
